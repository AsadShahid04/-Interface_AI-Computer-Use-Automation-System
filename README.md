...
A real LLM-driven discovery run was completed on 2026-09-08 using Claude Haiku. The artifact and structured logs are in `evidence/`.

To run your own discovery:
...
- **Real discovery artifact** (`artifacts/member-lookup-capability.json`) - Generated by Claude Haiku on 2026-09-08
- **Discovery logs** (`logs/discovery-haiku.json`) - LLM observations, decisions, and actions
...
**Completed**: A real LLM-driven discovery run was executed on 2026-09-08T21:34:09Z using Claude Haiku (`claude-haiku-4-5-20251001`).

The artifact (`evidence/artifacts/member-lookup-capability.json`) contains genuine metadata:
- `metadata.discoveryModel: "claude-haiku-4-5-20251001"`
- `metadata.discoveryRun: "2026-09-08T21:34:09.242Z"`

Discovery logs (`evidence/logs/discovery-haiku.json`) show the complete LLM decision-making process:
- Step-by-step observations of the UI
- Action decisions with reasoning
- Final goal achievement with extracted data

The agent:
- Used Claude Haiku for cost-optimized exploration
- Enforced safety policy (domain allowlist, risky action blocking)
- Captured structured logs with timestamps and context
- Generated a working artifact verified through replay
